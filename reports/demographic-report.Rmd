...
COLORS <- c("#22323e", "#22323e", "#2a587c", "#416b8b", "#dfe8f1", "#cdd0df")
```
...
df_data <- df_processed %>%
  ...
  mutate(name = get_question_text(name),
         ...
         name = sapply(name, function(x){paste(strwrap(x, width = 64), collapse = "<br>")}, USE.NAMES = FALSE),
         name = unname(name)) %>%
...
x1 <- c(21, 24, 27, 29) ##strongly agree
x2 <-c(30, 31, 26, 24)
x3 <- c(21, 19, 23, 15)
x4 <- c(16, 15, 11, 18)
x5 <- c(12, 11, 13, 14) #strongly disagree

top_labels <- c('Strongly<br>agree', 'Agree', 'Neutral', 'Disagree', 'Strongly<br>disagree')
top_labels <- get_question_labels("z1")
...
fig <- fig %>% add_trace(x = ~x6, marker = list(color = 'rgba(190, 192, 213, 1)'))
